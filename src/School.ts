...
declare interface School {
    on<U extends keyof SchoolEvents>(event: U, listener: SchoolEvents[U]): this;

    emit<U extends keyof SchoolEvents>(
        event: U,
        ...args: Parameters<SchoolEvents[U]>
    ): boolean;
}
...
class School extends EventEmitter {
    ...
    public async setup() {
        ...
        console.log(
            `Added ${Object.keys(this.classes).length} classes and ${
                Object.keys(this.classrooms).length
            } classrooms in ${Date.now() - st}ms`
        );
    ...
    private parseToUsable(classtt: Timetable[], clasroomtt: Timetable[]) {
        ...
        try {
            this.classTimetables = new Map(
                classtt.map((tt) => {
                    ...
                    const days: Day<ClassLesson>[] = tt.days.map((day) => ({
                        date: day.date,
                        lessons: day.lessons.map((period) =>
                            period.map((lesson) => ({
                                name: lesson.name ?? "?",
            ...
            this.classroomTimetables = new Map(
                clasroomtt.map((tt) => {
                    const { scheduleDefinitions, week } = tt;

                    const room = this.roomNameFromId(tt.classroomId ?? -1);

                    const days: Day<RoomLesson>[] = tt.days.map(
                        (day, dayIndex) => ({
                            date: day.date,
                            lessons: day.lessons.map((period, periodIndex) =>
                                period.map((lesson) => {
                                    //find the lesson in the classTimetable map

                                    const fullLessonObj = this.classTimetables
                                        .get(lesson.name ?? "")
                                        ?.days?.[dayIndex]?.lessons?.[
                                            periodIndex
                                        ].find(
                                            (x) => x.teacher === lesson.teacher
                                        );
            ...
            this.teachers = [
                ...new Set(
                    classtt.flatMap((tt) =>
                        tt.days.flatMap((day) =>
                            day.lessons.flatMap((period) =>
                                period.flatMap((lesson) => lesson.teacher)
                            )
                        )
                    )
                ),
            ...
            this.teachers.forEach((teacher) => {
                //construct the teacher timetables
                this.teacherTimetables.set(teacher, {
                    teacher: teacher,
                    week,
                    scheduleDefinitions,
                    days: [...Array(days.length)].map((d, di) => ({
                        date: [...this.classTimetables.values()][0].days[di]
                            .date,
                        lessons: [...Array(scheduleDefinitions.length)].map(
                            (x) => Array()
                        ),
                    })),
                });
            ...
            this.classTimetables.forEach((classtt) => {
                //fill in the teacher timetables
    ...
    private async scrapeAll() {
        ...
        const r = await Promise.allSettled([
            Promise.allSettled(
                cr.map((classroom, i) =>
                    delayStart(getTimetable, i * 100, 0, classroom, this.id)
                )
            ),
            Promise.allSettled(
                cs.map((sClass, i) =>
                    delayStart(getTimetable, i * 100 + 50, sClass, 0, this.id)
                )
            ),
        ]);
        if (r[0].status == "rejected" || r[1].status == "rejected") return;
        const rr = r as PromiseFulfilledResult<
            PromiseSettledResult<Timetable>[]
        >[];
        console.log(`Settled all promises in: ${Date.now() - st}ms`);
        console.log(
            `\tClasses:    [ ${
                r[1].value.filter((x) => x.status == "fulfilled").length
            } / ${cs.length} ]`
        );
        console.log(
            `\tClassrooms: [ ${
                r[0].value.filter((x) => x.status == "fulfilled").length
            } / ${cr.length} ]`
        );

        const [clasroomtt, classtt] = rr.map((ttCategory) =>
            (
                ttCategory.value.filter(
                    (tt) => tt.status === "fulfilled"
                ) as PromiseFulfilledResult<Timetable>[]
